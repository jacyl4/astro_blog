...
- 内容读取已收口到 `src/modules/blog`，页面不再直接依赖旧 BlogService。
- 浏览器端副作用已统一由 PageController lifecycle runtime 管理。
- 评论 UI、脚本、样式、Worker、D1、GitHub OAuth App、Actions secret 和公开域名均已删除；D1 按所有者决定直接删除且不备份。
- `astro-blog-staging` 已作为纯 Static Assets Worker 发布到 `https://blog-staging.seso.icu`。
...
| 当前 Pages 项目 | `blog`（生产观察期回退入口） |
| 已退役评论 Worker/域名 | `astro-blog-comments` / `astro-blog-comments.seso.icu` |
| 目标静态 Worker 名称 | production `astro-blog`；staging `astro-blog-staging` |
| staging 域名 | `https://blog-staging.seso.icu` |
| Node 版本 | `24.x` |
| npm 包管理器 | `npm` |
