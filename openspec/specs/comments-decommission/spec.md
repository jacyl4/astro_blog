# comments-decommission Specification
...
系统 MUST 在记录必要资源清单并执行所有者批准的数据处置决定后，删除 Comments Worker、route/custom domain、D1、OAuth App 凭据、CI secret/变量和 DNS。数据处置决定可以是导出，也可以是明确批准的不备份直接删除；本次契约采用后者。
