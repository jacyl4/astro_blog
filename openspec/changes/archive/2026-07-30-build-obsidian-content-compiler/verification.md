...
- 新 trigger 已完成三次真实 production；legacy adapter 和应用仓内容镜像已删除。
- 删除提交 `24f97e05…` 的功能分支 Pipeline `#441` 与默认分支 Pipeline `#442`
  均通过；最终 staging `#774`、production `#775` 成功。
- 本地和 production Playwright 均为 `11/11`，外部 HTTP 为 `86 + 3`，
  production manifest 为 app `24f97e05…` / content `7a5b1257…`。
