...
gRPC 默认使用 HTTP/2 作为其传输协议 。而 HTTP/2 的标准规范及其绝大多数实现都运行在 TCP 之上 。因此，当一个 gRPC 隧道被设计用于封装和传输 TCP 数据报时（如 OpenConfig 的 grpctunnel ），内部的 TCP 会话就会被隧道化到 gRPC 的外部 TCP 会话中。这构成了典型的 TCP-over-TCP 场景。  
